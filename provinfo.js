...
(function($, window, document, undefined) {
    ...
    function getHouseHoldId() {
        window.parent.$("iframe").each(function(){
            if ($(this).contents().find("label[for='HouseholdId']").length !== 0) {
                var ff = $(this).contents().find("label[for='HouseholdId']").context.id;
                houseHoldNum = window.parent.$('iframe[id='+ff+']').contents().find("label[for='HouseholdId']")[0].nextElementSibling.children[0].innerHTML;
                sessionStorage.setItem('mandrHouseholdId', houseHoldNum);
            }
            houseHoldNum = sessionStorage.getItem('mandrHouseholdId');
        });
    ...
    var interaction_id = window.parent.$('#EXPAND-OUTERFRAME').find('.header-content').find('.standard_label_bold_dataLabelWrite').find('nobr').find('label')[0].innerHTML.slice(16);

    var varSectionIndex = "#pyFlowActionHTML div div[class='layout layout-noheader layout-noheader-default_with_all_borders'";

    window.parent.sessionStorage.setItem(interaction_id + "varSectionIndex",varSectionIndex);
    ...
    $(document).on('DOMSubtreeModified', '#pyFlowActionHTML div ', function() {
        if ($("#newlyAddedQuestionEmail").length == 0 && document.forms[0].elements["TaskSectionReference"].value == "AssignPCP") {
            var storedSectionIndex = window.parent.sessionStorage.getItem(interaction_id + "varSectionIndex");
            console.info("storedSectionIndex - " + storedSectionIndex);
            if (window.parent.sessionStorage.getItem("QuestionradioStatus") == "OPT_IN"){
                $(storedSectionIndex).prev().prepend('<tr id="newlyAddedQuestionEmail"><td><label class="dataValueWrite a4meDiv" style="vertical-align:middle;">Does the member want to receive provider information via text or email?</label></td>' + EmailCheckRadioButtonContentYes + '</tr>');
            }else if  (window.parent.sessionStorage.getItem("QuestionradioStatus") == "OPT_OUT") {
                $(storedSectionIndex).prev().prepend('<tr id="newlyAddedQuestionEmail"><td><label class="dataValueWrite a4meDiv" style="vertical-align:middle;">Does the member want to receive provider information via text or email?</label></td>' + EmailCheckRadioButtonContentNo + '</tr>');
            }else {
                $(storedSectionIndex).prev().prepend('<tr id="newlyAddedQuestionEmail"><td><label class="dataValueWrite a4meDiv" style="vertical-align:middle;">Does the member want to receive provider information via text or email?</label></td>' + EmailCheckRadioButtonContent + '</tr>');
            }
        }
    });
